  ...
  if (isset($_SESSION['Ownerid'])) {
    ...
    if (isset($_POST['dispatcheddelivery'])) {
      $currenttime=time();
      $orderid=$_POST['orderid'];
      $emp=$_POST['empid'];
      $query = "UPDATE finalorder SET deliveryboy=$emp,timedispatch=$currenttime WHERE o_id=$orderid";
      $result = mysqli_query($con, $query);
      echo "<script type='text/javascript'>
      alert('Order ".$oerderid." is dispatched.');
  ...
  <br><br>
  <?php if ($deliver==1) {?>
  <h4>Assign a Delivery Boy</h4><br>
