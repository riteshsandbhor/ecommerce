...
<div class="card ownercard">
  ...
  <tbody>
    <?php
    $totalfinal=0;
    $query2 = "SELECT o_id FROM finalorder WHERE delivery=1 && deliveryboy=$id && timedelivery BETWEEN $timeStamp AND $timeStamp2";
...
<div class="card ownercard">
  ...
  <tbody>
    <?php
    $query = "SELECT * FROM employee WHERE d_id=$id;";
    $result = mysqli_query($con, $query);
    if (mysqli_num_rows($result) > 0)
    {
        while ($row1 = mysqli_fetch_assoc($result)) {
